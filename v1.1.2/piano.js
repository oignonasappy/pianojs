...
function pianoLoad() {
    ...
    pianos.forEach(piano => {
        ...
        piano.last = parseInt(piano.dataset.last) || piano.first + 12;
...
function midiToNoteName(midi, keySignature = 'C maj', hasOctave = true) {
    ...
    if (hasOctave) {
        ...
        return TONALITY_LIST_FLAT[tonality][Math.round(midi) % 12] + octave;
    }
    return TONALITY_LIST_FLAT[tonality][Math.round(midi) % 12];
}
